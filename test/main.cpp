...
int main (int argv, char **argc) {
	...
	std::map< std::string, int > domain_cats;
	...
	m_c.connect("localhost");
	
	std::auto_ptr<mongo::DBClientCursor> cursor = m_c.query("local.pages", "{}");
	...
	while (cursor->more()) {
		...
		std::string domain = bdomain.str();
		std::string url = obj.getField("_id").str();
		
		if (crc32(0, url.c_str(), url.size()) % 2 != 0)
			continue;
		
		mongo::BSONElement btext = obj.getField("text");
		std::string text = btext.str();
		
		auto dom_cat_it = domain_cats.find(domain);
		if (dom_cat_it == domain_cats.end()) {
			std::cout << "no such domain: " << domain << std::endl;
			continue;
		}
	
		int real_category =  dom_cat_it->second;
		
		int cat = c.getCategory(text);
		...
		if (ndocs % 100 == 0)
			std::cout << "ndocs: " << ndocs << " matches:" << nmatches << std::endl;
		
	}
	
	std::cout << "accuracy: " << float(nmatches)/ndocs << std::endl;
